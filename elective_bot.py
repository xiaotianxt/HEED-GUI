...
class ElectiveBot:
        ...
        if "<title>帮助-总体流程</title>" not in res.text:
            if "/scnStAthVef.jsp/" in res.text:
                if not auth["channel"]:
                    ...
                    self.log(
                        "debug",
                        "choose channel %s for sida %s" % (auth["channel"], sida),
                    )
    ...
    def proc_course_elem(self, rows):
        for row in rows:
            ...
            volume_cnt, _, elected_cnt = (
                row.select('td span[id^="electedNum"]')[0]
                .get_text(strip=True)
                .partition(" / ")
            )
            if "/" in elected_cnt:
    ...
    def loop_(self, url=None):
        ...
        if fatal_err:
            if (
                "目前不是补退选时间" not in fatal_err
                and "目前是跨院系选课数据准备时间" not in fatal_err
                and "目前时段不能选课" not in fatal_err
            ):
                self.status = "dead"
        ...
        courses = list(
            self.proc_course_elem(
                soup.select("tr.datagrid-all, tr.datagrid-odd, tr.datagrid-even")
            )
        )
